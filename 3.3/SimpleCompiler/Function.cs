    ...
    public class Function : JackProgramElement
    {
        ...
        public override void Parse(TokensStack sTokens)
        {
            ...
            while(sTokens.Count > 0 && !(sTokens.Peek() is Parentheses))//)
            {
                ...
                if(!(tArgType is VarType))
                    throw new SyntaxErrorException("Expected Type received: " + tArgType, tArgType);
                if(!(tArgName is Identifier))
                    throw new SyntaxErrorException("Expected ID received: " + tArgName, tArgName);
            ...
            if(sTokens.Count == 0)
                throw new SyntaxErrorException("Expected } received: empty", sTokens.LastPop);
